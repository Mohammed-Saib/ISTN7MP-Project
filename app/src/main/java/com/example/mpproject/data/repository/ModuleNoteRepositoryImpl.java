...
// [Data] Implements ModuleNoteRepository.
// Room is the source of truth; Firestore is synced in the background.
public class ModuleNoteRepositoryImpl implements ModuleNoteRepository {
    ...
    public void syncFromFirestore(String userId) {
        firestore.collection("users").document(userId)
                    ...
                    for (com.google.firebase.firestore.DocumentSnapshot moduleDoc : modulesSnap.getDocuments()) {
                        ...
                        firestore.collection("users").document(userId)
                                ...
                                .addOnFailureListener(e -> Log.e(
                                        TAG,
                                        "Firestore module_notes sync failed for module " + moduleId,
                                        e
                                ));
                    }
                })
                .addOnFailureListener(e -> Log.e(
                        TAG,
                        "Firestore modules fetch failed during module note sync",
                        e
                ));
    }
    ...
    private List<ModuleNote> toDomainList(List<ModuleNoteEntity> entities) {
        ...
        if (entities != null) {
            for (ModuleNoteEntity e : entities) {
                list.add(toDomain(e));
            }
        }

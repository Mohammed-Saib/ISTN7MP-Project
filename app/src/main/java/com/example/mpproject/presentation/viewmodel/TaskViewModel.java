...
public class TaskViewModel extends ViewModel {
    ...
    public TaskViewModel(TodoRepository todoRepository, ModuleRepository moduleRepository) {
        ...
        mediumPriorityTodos = Transformations.map(allTodos, list -> {
            ...
            for (Todo t : list) {
                if ("MEDIUM".equals(t.getPriority())) {
                    out.add(t);
                }
            }
    ...
    public void setEditingTodo(Todo todo) {
        editingTodo.setValue(todo);
    }

    public Todo addTask(String title, String description, String priority,
                        Long dueDate, String moduleId,
                        String recurrencePattern, Long recurrenceEndDate) {

        boolean isRecurring = recurrencePattern != null && !recurrencePattern.trim().isEmpty();
        String groupId = isRecurring ? UUID.randomUUID().toString() : null;
        ...
        todo.setRecurrencePattern(isRecurring ? recurrencePattern : null);
        todo.setRecurrenceGroupId(groupId);
        todo.setRecurrenceEndDate(isRecurring ? recurrenceEndDate : null);
    ...
    public void updateTask(Todo todo) {
        ...
        if (todo.getRecurrenceGroupId() != null) {
            /*
             * Propagate non-date fields to all occurrences,
             * then update this instance's own row.
             */
    ...
    // Toggle completion; if recurring and just completed, create the next occurrence.
    public void toggleTask(Todo todo, boolean checked) {
        ...
        if (checked
                && todo.getRecurrencePattern() != null
                && todo.getRecurrenceGroupId() != null) {
     ...
     * Notification scheduling stays in the View/Presentation interaction layer because
     * this ViewModel should not hold an Android Context.
     */
    private void scheduleNextOccurrence(Todo completed) {
        if (completed.getDueDate() == null) return;
        if (completed.getRecurrencePattern() == null) return;
        ...
        if (completed.getRecurrenceEndDate() != null
                && nextMs > completed.getRecurrenceEndDate()) {
            return;
        }
        ...
        nextTodo.setUpdatedAt(System.currentTimeMillis());
